  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    if (!collide && moved && mx !== null && my !== null && particle.dist < r) {
      ...
      if (particle.dist > particle.r) {
        ...
        particle.ang =
          -Math.atan2(particle.y - MY, particle.x - MX) + 0.5 * Math.PI;
      }
      _break = true;
    } else {
      particle.sp = 0.25;
...
function checkBorderCollision(particle) {
  ...
  if (
    particle.y - particle.r < -_camY ||
    particle.y + particle.r > canvas.height - _camY
  ) {
    reflect_v(particle);
    collide = true;
  }
  if (
    particle.x - particle.r < -_camX ||
    particle.x + particle.r > canvas.width - _camX
  ) {
    reflect_h(particle);
    collide = true;
  }
...
function reflect_h(particle) {
  var ang = -particle.ang,
    x =
      -Math.sin(ang) * Math.cos((180 / 180) * Math.PI) -
      Math.cos(ang) * Math.sin((180 / 180) * Math.PI),
    y =
      -Math.cos(ang) * Math.cos((180 / 180) * Math.PI) -
      Math.sin(ang) * Math.sin((180 / 180) * Math.PI);
